...
import { HiSearch, HiVideoCamera, HiDotsVertical, HiPhoneOutgoing } from "react-icons/hi";
  ...
  return (
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className='bg-no-repeat bg-cover'>
      <header className='sticky top-0 h-16 bg-white flex justify-between items-center px-4'>
        ...
        <div>
          <button className='cursor-pointer hover:text-primary mr-6' onClick={() => handleCompletedCall({ time: new Date() })}>
            <AudioCallButton />
          </button>
          <VideoCallButton />
